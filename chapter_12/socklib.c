/* socklib.c
 * This file contains functions used lots whenb writing internet 
...
int make_server_socket_q(int portnum, int backlog)
{
	...
	struct hostent *hp;				//this is part of pur
	...
	saddr.sin_port = htons(portnum);            //将主机字节序转换为网络字节序
												//然后再填充套接字端口号
	sadr.sin_family = AF_INET;					//fill in family
	if(bind(sock_id,(struct sockaddr *)&saddr,sizeof(saddr)) != 0)
		reurn -1;
	//arange for incoming calls
...
{
	int sock;
	struct sockaddr_in servadd; 		//the numner to call
	...
	if(connect(sock,(struct socaddr *)&servadd, sizeof(servadd)) != 0)
		return -1;
	return sock;
}