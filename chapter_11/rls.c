 ...
 */
 ...
 int main(int ac, char *av[])
 {
	...
	char buffer[BUFSIZ];			//to receive messag
	int  n_read;					//for message length
	
	if(ac != 3) exit(1);
	
	//Step 1: Get a socket
	sock_id = socket(AF_INET,SOCK_SEREAM,0);	//get a line 
	if(sock_id == -1)
		oops("socket");							//or fail 
	
	//step 2: connect to server
    bzero(&servadd, sizeof(servadd));			//zero the adress
	...
	//Step 3: send directory name,then red back results
